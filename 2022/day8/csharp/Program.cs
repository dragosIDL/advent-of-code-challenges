...
Console.WriteLine($"part1={trees.Count(a => a.isVisible) + rows + sides}");
Console.WriteLine($"part2={trees.Max(a => a.score)}");

static int CountVisibleTrees(List<int> allTrees, int currentTree)
{
    var visibleTrees = allTrees.TakeWhile(i => i < currentTree).ToList();
    var seesToTheEdge = visibleTrees.Count == allTrees.Count;
    return visibleTrees.Count + (seesToTheEdge ? 0 : 1);
}